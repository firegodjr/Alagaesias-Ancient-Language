...
public class WordHandler {
	...
	/**
	 * Gets the index of an action word if it's stored in String[] "livingTargets".
	 * Returns -1 if index is unavailable.
	 * @param word
	 * @return index
	 */
	...
	/**
	 * Gets the index of an action word if it's stored in String[] "blockTargets".
	 * Returns -1 if index is unavailable.
	 * @param word
	 * @return index
	 */
	...
	public static List<BlockPosHit> getBlockTargetsFromWord(String word, ICommandSender sender)
	{
		...
		case 0: //sem, or that
			...
			{
				MovingObjectPosition pos = new MovingObjectPosition(MovingObjectType.BLOCK, new Vec3(0,0,0), EnumFacing.UP, ((WardEntity) sender).getPos());
			}
			break;
	...
	public static List GetSpecificBlockTargets(ICommandSender sender, Block block)
	{
		...
		else blockList.addAll(getBlocks(block, 2, sender.getPosition()));
		return blockList;
	}
	...
	public static EntListIterated getLivingTargetsFromWord(String word, ICommandSender sender)
	{
		...
		/*
		 * Find the target entity to apply an action to
		 * Can only be a pronoun from the ancient language
		 * Default is "edtha" or "me", which applies to the sender
		 */
		if(!world.isRemote)
		{
			switch(wordIndex)
	...
	/**
	 * Performs an action against the passed in EntityLivingBase reference, then calls tirePlayer(player, (calculated fatigue), distance)
	 * @param sender
	 * @param actionWord
	 * @param targetList
	 * @param distance
	...
	public static void performEntityAction(ICommandSender sender, String actionWord, List<EntityLivingBase> targetList, int distance)
	{
		...
		for(EntityLivingBase ent : targetList)
		{
			if(ent == sender)
			{
				...
				{
					EntityPlayer player = (EntityPlayer) sender;
					player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.GRAY + "" + EnumChatFormatting.ITALIC + ent.getName() + " flinches as you say the word \"" + EnumChatFormatting.RESET + EnumChatFormatting.AQUA + actionWord + EnumChatFormatting.GRAY + "" + EnumChatFormatting.ITALIC + "\"."));
				}
			if(ent instanceof EntityPlayer)
			{
				((EntityPlayer) ent).addChatComponentMessage(new ChatComponentText(EnumChatFormatting.GRAY + "" + EnumChatFormatting.ITALIC + "The word \"" + EnumChatFormatting.RESET + EnumChatFormatting.AQUA + actionWord + EnumChatFormatting.GRAY + "" + EnumChatFormatting.ITALIC + "\" echoes in your mind..."));
			}
		}
	}
	
	/**
	 * Performs an action against the passed in BlockPos's, then calls tirePlayer
	 * @param sender
	 * @param word
	 * @param blockTargets
	 */
		...
		if(!sender.getEntityWorld().isRemote)
			...
			case 8: //risa, or rise
				for(int i = 0; i < blockTargets.size(); i++)
				{
					...
					EntityFallingBlock blockEntity = new EntityFallingBlock(world, pos.getX()+0.5, pos.getY()+0.5, pos.getZ()+0.5, state);
	...
	/**
	 * Finds entities within a certain radius using player.worldObj.getEntitiesWithinAABB
	 * @param sender
	 * @param radius
	 * @return
	 */
	private static List findEntitiesWithinRadius(ICommandSender sender, int radius)
	{
		...
		entList = sender.getEntityWorld().getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB(sender.getPosition().add(-radius, -(int)radius/3, -radius), sender.getPosition().add(radius, (int)radius/3, radius)));
		return entList;
	}
	
	/**
	 * Finds the nearest entities that extend EntityLiving by iterating from startRadius to maxRadius
	 * @param sender
	 * @param startRadius
	 * @param maxRadius
	 ...
	 * @param startRadius
	 * @param maxRadius
	 * @return An EntListIterated, containing only the first EntityPlayer found and the iterations
	 */
	...
	/**
	 * Gets all nearby blocks matching the passed in block, and also returns the side facing the centerpoint
	 * @param block
	 * @param radius
	 * @param centerpoint
...
//						}
	...
	/**
	 * Applies a hunger potion effect with a potency based on the amount of fatigue passed in; If hunger isn't full,
	 * damages the player with damage = 1/2 heart * fatigue + empty hunger drumsticks
	 * @param sender
	 * @param fatigue
	 * @param distance
	 */
		...
		if(sender instanceof EntityPlayer)
		{
			...
			if(!player.capabilities.isCreativeMode)
				...
				if(player.isPotionActive(Potion.hunger.id))
				{
					...
					player.addPotionEffect(new PotionEffect(Potion.hunger.id, 5, duration + 30 + 10*(int)fatigue + 5*(int)distance, true, false));
				}
				else
				{
					player.addPotionEffect(new PotionEffect(Potion.hunger.id, 10 + 10*(int)fatigue, 30 + 5*(int)distance, true, false));
				}
				
				if(player.isPotionActive(Potion.moveSlowdown.id))
				{
					int duration = player.getActivePotionEffect(Potion.moveSlowdown).getDuration();
					player.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 1 + (int)(fatigue/2), duration + 20 + 20*(int)distance));
				}
				else
				{
					player.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 1 + (int)(fatigue/2), 20 + 20*(int)distance));
				}
		}
		else if(sender instanceof WardEntity)
		{
